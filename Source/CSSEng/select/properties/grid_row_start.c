...
{
	return css__cascade_length_auto(opv, style, state, set_grid_row_start);
}

css_error css__set_grid_row_start_from_hint(const css_hint *hint,
		css_computed_style *style)
{
	return set_grid_row_start(style, hint->status,
			hint->data.length.value, hint->data.length.unit);
}

css_error css__initial_grid_row_start(css_select_state *state)
{
	return set_grid_row_start(state->computed, CSS_WIDTH_AUTO, 0, CSS_UNIT_PX);
}
		...
		css_computed_style *result)
{
	...
	uint8_t type = get_grid_row_start(child, &length, &unit);

	if (type == CSS_WIDTH_INHERIT) {
		type = get_grid_row_start(parent, &length, &unit);
	}

	return set_grid_row_start(result, type, length, unit);
}

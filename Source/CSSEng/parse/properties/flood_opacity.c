		...
		css_style *result, int np)
{
	...
	if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(token->idata, c->strings[INHERIT], &match) == lwc_error_ok && match)) {
			error = css_stylesheet_style_inherit(result, CSS_PROP_FLOOD_OPACITY);
		...
		error = css__stylesheet_style_appendOPV(result, CSS_PROP_FLOOD_OPACITY, 0, FLOOD_OPACITY_SET);
		if (error != CSS_OK) {
			*ctx = orig_ctx;
			return error;
		}

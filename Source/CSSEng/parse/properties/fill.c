        ...
        css_style *result, int np)
{
    ...
    if ((token->type == CSS_TOKEN_IDENT) &&
        ...
        error = css__stylesheet_style_appendOPV(result,
                CSS_PROP_FILL, 0, FILL_NONE);
    } else if ((token->type == CSS_TOKEN_IDENT) &&
            (lwc_string_caseless_isequal(token->idata,
                                         c->strings[CURRENTCOLOR], &match) ==
             lwc_error_ok && match)) {
        error = css__stylesheet_style_appendOPV(result,
                CSS_PROP_FILL, 0, FILL_CURRENT_COLOR);
        ...
        *ctx = orig_ctx;
        ...
        error = css__stylesheet_style_appendOPV(result, CSS_PROP_FILL, 0, FILL_SET_COLOR);
        if (error != CSS_OK) {
            *ctx = orig_ctx;
            return error;
        }

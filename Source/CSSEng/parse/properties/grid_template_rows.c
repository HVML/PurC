        ...
        css_style *result, int np)
{
    ...
    while ((token = parserutils_vector_iterate(vector, ctx)) != NULL) {
        if (token->idata != NULL) {
            ...
            error = css__stylesheet_style_appendOPV(result, CSS_PROP_GRID_TEMPLATE_ROWS, 0, GRID_TEMPLATE_ROWS_SET);
            if (error != CSS_OK)
            {
                *ctx = orig_ctx;
                return error;
    ...
    error = css__stylesheet_style_append(result, GRID_TEMPLATE_ROWS_END);
    return error;
}

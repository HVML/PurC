/*
 * @file op_greater_equal.c
 * @author XueShuming
 * @date 2025/08/21
 * @brief The impl of ops for greater equal operator vcm node.
 *
        ...
        struct pcvcm_eval_stack_frame *frame, const char **name)
{
    ...
    int cmp = purc_variant_compare_ex(left, right, PCVRNT_COMPARE_METHOD_AUTO);
    
    // Return true if left >= right (cmp >= 0)
    return purc_variant_make_boolean(cmp >= 0);
}

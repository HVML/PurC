...
pcintr_get_scope_ns_vars(purc_coroutine_t cor, pcvdom_element_t elem)
{
    ...
    struct rb_node *p;
    struct rb_node *first = pcutils_rbtree_first(&stack->scoped_ns_vars);
    pcutils_rbtree_for_each(first, p) {
        struct pcns_varmgr *mgr = container_of(p, struct pcns_varmgr, node);
        if (mgr->vdom_node == node) {
            return mgr;
        }
    }
...
static int
cmp_ns(struct rb_node *node, void *ud)
{
    struct pcns_varmgr *mgr = container_of(node, struct pcns_varmgr, node);
    PC_ASSERT(mgr->vdom_node);
    PC_ASSERT(ud);
    struct pcvdom_node *v = (struct pcvdom_node*)ud;
    if (mgr->vdom_node < v)
...
static struct rb_node*
new_ns_varmgr(void *ud)
{
    PC_ASSERT(ud);
    struct pcvdom_node *v = (struct pcvdom_node*)ud;

    struct pcns_varmgr *mgr = pcns_varmgr_create();
    if (!mgr)
        return NULL;
...
static struct pcns_varmgr *
create_scoped_ns_vars(purc_coroutine_t cor, struct pcvdom_node *node)
{
    pcintr_stack_t stack = &cor->stack;

    /* vdom level manage by coroutine */
    if (node == (void*)stack->vdom) {
        return stack->co->ns_vars;
    }

    struct rb_node *p;
    int r = pcutils_rbtree_insert_or_get(&stack->scoped_ns_vars, node,
            cmp_ns, new_ns_varmgr, &p);
    if (r) {
        purc_set_error(PURC_ERROR_OUT_OF_MEMORY);
        return NULL;
    }
    ...
    return container_of(p, struct pcns_varmgr, node);
}
        ...
        struct pcns_varmgr **mgr)
{
    ...
    struct pcns_varmgr *ns_varmgr = create_scoped_ns_vars(cor, node);
    if (!ns_varmgr) {
        return false;
    }
    ...
    return pcns_varmgr_add(ns_varmgr, name, variant, ns);
}
        ...
        const char* name, purc_variant_t ns)
{
    ...
    struct pcns_varmgr *ns_varmgr = pcintr_get_scope_ns_vars(cor, elem);
    if (!ns_varmgr) {
        return false;
    }

    return pcns_varmgr_remove(ns_varmgr, name, ns);
}
        ...
        const char* name, purc_variant_t ns)
{
    ...
    return pcns_varmgr_get(ns_varmgr, name, ns);
}

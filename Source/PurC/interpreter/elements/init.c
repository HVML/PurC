        ...
        purc_variant_t src)
{
    ...
    if (as) {
        ...
        pcintr_set_result_var(frame, src);
    }
    else {
        pcintr_set_result_var(frame, src);
    }
...
params_from_with(struct ctxt_for_init *ctxt, pcintr_stack_frame_t frame)
{
    ...
    if (!with) {
        with = pcintr_get_symbol_var(frame, PURC_SYMBOL_VAR_CNT);
    }
        ...
        const char *sub_type)
{
    ...
    if (strcmp(type, MSG_TYPE_FETCHER_STATE) == 0) {
        match = true;
        goto out;
    }
...
after_pushed(pcintr_stack_t stack, pcvdom_element_t pos)
{
    ...
    if (ctxt->with) {
        r = pcintr_set_result_var(frame, ctxt->with);
        if (r == 0)
        ...
        struct pcvdom_content *content)
{
    ...
    purc_variant_t v = pcintr_get_symbol_var(frame, PURC_SYMBOL_VAR_CNT);

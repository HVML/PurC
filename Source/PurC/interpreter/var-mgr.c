...
static void *
ns_mgr_copy_val(const void *datum)
{
    return purc_variant_ref((purc_variant_t)datum);
}

static void
ns_mgr_free_val(void *datum)
{
    purc_variant_unref((purc_variant_t)datum);
}

struct pcns_varmgr *
pcns_varmgr_create(void)
{
    struct pcns_varmgr *mgr = (struct pcns_varmgr *)calloc(1, sizeof(*mgr));
    if (!mgr) {
        purc_set_error(PURC_ERROR_OUT_OF_MEMORY);
        goto out;
    }

    mgr->map = pcutils_map_create(copy_key_string, free_key_string,
                          ns_mgr_copy_val, ns_mgr_free_val,
...
int
pcns_varmgr_destroy(struct pcns_varmgr *ns_mgr)
{
    if (ns_mgr) {
        pcutils_map_destroy(ns_mgr->map);
        free(ns_mgr);
    }
    return 0;
}

#define DEFAULT_NS_NAME         "1"

static pcvarmgr_t
find_by_ns(struct pcns_varmgr *ns_mgr, purc_variant_t ns)
{
    pcvarmgr_t mgr = NULL;
    if (!ns) {
        pcutils_map_entry *entry = pcutils_map_find(ns_mgr->map, DEFAULT_NS_NAME);
        if (entry) {
            mgr = entry->val;
        }
    }
    else if (purc_variant_is_string(ns)){
        const char *name = purc_variant_get_string_const(ns);
        pcutils_map_entry *entry = pcutils_map_find(ns_mgr->map, name);
        if (entry) {
            mgr = entry->val;
        }
    }
    else if (purc_variant_is_ulongint(ns)) {
        char *name = NULL;
        ssize_t nr = purc_variant_stringify_alloc(&name, ns);
        if (nr == -1) {
        ...
        purc_variant_t value, purc_variant_t ns)
{
    ...
    if (!mgr) {
        ...
        int r = pcutils_map_insert(ns_mgr->map, DEFAULT_NS_NAME, mgr);
        if (r) {
    ...
    ret = pcvarmgr_add(mgr, name, value);

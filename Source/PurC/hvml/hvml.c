...
#define POP_AS_VCM_PARENT_AND_UPDATE_VCM()                                  \
    do {                                                                    \
        struct pcvcm_node* parent = pcvcm_stack_pop(hvml->vcm_stack);       \
        struct pcvcm_node* child = hvml->vcm_node;                          \
        APPEND_CHILD(parent, child);                                        \
        UPDATE_VCM_NODE(parent);                                            \
    } while (false)

#define ejson_stack_is_empty()  pcutils_stack_is_empty(hvml->ejson_stack)
#define ejson_stack_top()  pcutils_stack_top(hvml->ejson_stack)
#define ejson_stack_pop()  pcutils_stack_pop(hvml->ejson_stack)
#define ejson_stack_push(c) pcutils_stack_push(hvml->ejson_stack, c)

#define vcm_stack_is_empty() pcvcm_stack_is_empty(hvml->vcm_stack)
#define vcm_stack_push(c) pcvcm_stack_push(hvml->vcm_stack, c)
#define vcm_stack_pop() pcvcm_stack_pop(hvml->vcm_stack)
    ...
    switch (hvml->state) {
        ...
        BEGIN_STATE(PCHVML_ATTRIBUTE_VALUE_DOUBLE_QUOTED_STATE)
            ...
            if (character == '$') {
                ...
                if (!pchvml_buffer_is_empty(hvml->string_buffer)) {
                    ...
                    APPEND_CHILD(hvml->vcm_node, node);
                    RESET_STRING_BUFFER();
                }
                RECONSUME_IN(PCHVML_EJSON_DATA_STATE);
            }
        ...
        BEGIN_STATE(PCHVML_EJSON_FINISHED_STATE)
            if (is_whitespace(character) || character == '}' ||
                ...
                if (hvml->vcm_node) {
                    if (hvml->vcm_tree) {
                        APPEND_CHILD(hvml->vcm_tree, hvml->vcm_node);
                    }
                    else {

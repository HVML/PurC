...
BEGIN_STATE(PCHVML_BEFORE_ATTRIBUTE_VALUE_STATE)
    ...
    if (character == '"') {
        ADVANCE_TO(PCHVML_JSONEE_ATTRIBUTE_VALUE_DOUBLE_QUOTED_STATE);
    }
    if (character == '\'') {
        ADVANCE_TO(PCHVML_JSONEE_ATTRIBUTE_VALUE_SINGLE_QUOTED_STATE);
    }
    RECONSUME_IN(PCHVML_JSONEE_ATTRIBUTE_VALUE_UNQUOTED_STATE);
END_STATE()

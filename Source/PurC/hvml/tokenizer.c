...
BEGIN_STATE(TOKENIZER_ATTRIBUTE_VALUE_DOUBLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_ATTRIBUTE_VALUE_SINGLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_ATTRIBUTE_VALUE_UNQUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_AFTER_ATTRIBUTE_VALUE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_SELF_CLOSING_START_TAG_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_BOGUS_COMMENT_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_MARKUP_DECLARATION_OPEN_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_COMMENT_START_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_COMMENT_START_DASH_STATE)
        ADVANCE_TO(TOKENIZER_COMMENT_END_STATE);
END_STATE()

BEGIN_STATE(TOKENIZER_COMMENT_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_COMMENT_LESS_THAN_SIGN_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_COMMENT_LESS_THAN_SIGN_BANG_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_COMMENT_LESS_THAN_SIGN_BANG_DASH_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_COMMENT_LESS_THAN_SIGN_BANG_DASH_DASH_STATE)
        RECONSUME_IN(TOKENIZER_COMMENT_END_STATE);
    RECONSUME_IN(TOKENIZER_COMMENT_END_STATE);
END_STATE()

BEGIN_STATE(TOKENIZER_COMMENT_END_DASH_STATE)
        ADVANCE_TO(TOKENIZER_COMMENT_END_STATE);
END_STATE()

BEGIN_STATE(TOKENIZER_COMMENT_END_STATE)
        ADVANCE_TO(TOKENIZER_COMMENT_END_STATE);
END_STATE()

BEGIN_STATE(TOKENIZER_COMMENT_END_BANG_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_DOCTYPE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_BEFORE_DOCTYPE_NAME_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_DOCTYPE_NAME_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_AFTER_DOCTYPE_NAME_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_AFTER_DOCTYPE_PUBLIC_KEYWORD_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_BEFORE_DOCTYPE_PUBLIC_ID_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_DOCTYPE_PUBLIC_ID_DOUBLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_DOCTYPE_PUBLIC_ID_SINGLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_AFTER_DOCTYPE_PUBLIC_ID_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_BETWEEN_DOCTYPE_PUBLIC_ID_AND_SYSTEM_INFO_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_AFTER_DOCTYPE_SYSTEM_KEYWORD_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_BEFORE_DOCTYPE_SYSTEM_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_DOCTYPE_SYSTEM_DOUBLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_DOCTYPE_SYSTEM_SINGLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_AFTER_DOCTYPE_SYSTEM_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_BOGUS_DOCTYPE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_CDATA_SECTION_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_CDATA_SECTION_BRACKET_STATE)
        ADVANCE_TO(TOKENIZER_CDATA_SECTION_END_STATE);
END_STATE()

BEGIN_STATE(TOKENIZER_CDATA_SECTION_END_STATE)
        ADVANCE_TO(TOKENIZER_CDATA_SECTION_END_STATE);
END_STATE()

BEGIN_STATE(TOKENIZER_CHARACTER_REFERENCE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_NAMED_CHARACTER_REFERENCE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_AMBIGUOUS_AMPERSAND_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_NUMERIC_CHARACTER_REFERENCE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_HEXADECIMAL_CHARACTER_REFERENCE_START_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_DECIMAL_CHARACTER_REFERENCE_START_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_HEXADECIMAL_CHARACTER_REFERENCE_STATE)
        ADVANCE_TO(TOKENIZER_NUMERIC_CHARACTER_REFERENCE_END_STATE);
END_STATE()

BEGIN_STATE(TOKENIZER_DECIMAL_CHARACTER_REFERENCE_STATE)
        ADVANCE_TO(TOKENIZER_NUMERIC_CHARACTER_REFERENCE_END_STATE);
END_STATE()

BEGIN_STATE(TOKENIZER_NUMERIC_CHARACTER_REFERENCE_END_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_SPECIAL_ATTRIBUTE_OPERATOR_IN_ATTRIBUTE_NAME_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_SPECIAL_ATTRIBUTE_OPERATOR_AFTER_ATTRIBUTE_NAME_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_DATA_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_FINISHED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_CONTROL_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_LEFT_BRACE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_RIGHT_BRACE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_LEFT_BRACKET_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_RIGHT_BRACKET_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_LEFT_PARENTHESIS_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_RIGHT_PARENTHESIS_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_DOLLAR_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_AFTER_VALUE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_BEFORE_NAME_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_AFTER_NAME_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_NAME_UNQUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_NAME_SINGLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_NAME_DOUBLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_VALUE_SINGLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_VALUE_DOUBLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_AFTER_VALUE_DOUBLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_VALUE_TWO_DOUBLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_VALUE_THREE_DOUBLE_QUOTED_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_KEYWORD_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_AFTER_KEYWORD_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_BYTE_SEQUENCE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_AFTER_BYTE_SEQUENCE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_HEX_BYTE_SEQUENCE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_BINARY_BYTE_SEQUENCE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_BASE64_BYTE_SEQUENCE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_VALUE_NUMBER_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_AFTER_VALUE_NUMBER_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_VALUE_NUMBER_INTEGER_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_VALUE_NUMBER_FRACTION_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_VALUE_NUMBER_EXPONENT_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_VALUE_NUMBER_EXPONENT_INTEGER_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_VALUE_NUMBER_SUFFIX_INTEGER_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_VALUE_NUMBER_INFINITY_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_VALUE_NAN_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_STRING_ESCAPE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_STRING_ESCAPE_FOUR_HEXADECIMAL_DIGITS_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_JSONEE_VARIABLE_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_JSONEE_FULL_STOP_SIGN_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_JSONEE_KEYWORD_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_JSONEE_STRING_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_AFTER_JSONEE_STRING_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_TEMPLATE_DATA_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_TEMPLATE_DATA_LESS_THAN_SIGN_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_TEMPLATE_DATA_END_TAG_OPEN_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_TEMPLATE_DATA_END_TAG_NAME_STATE)
END_STATE()

BEGIN_STATE(TOKENIZER_EJSON_TEMPLATE_FINISHED_STATE)
END_STATE()

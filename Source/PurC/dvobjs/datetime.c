        ...
        bool silently)
{
    ...
    return make_broken_down_time(&result, tv.tv_usec);
...
get_broken_down_time(purc_variant_t bdtime, struct tm *tm, suseconds_t *usec)
{
    ...
    *usec = (suseconds_t)number;
    ...
    val = purc_variant_object_get_by_ckey(bdtime, _KN_sec, false);
    if (val == PURC_VARIANT_INVALID) {
        goto failed;
    }
    if (!purc_variant_cast_to_number(val, &number, false) ||
    ...
    val = purc_variant_object_get_by_ckey(bdtime, _KN_min, false);
    if (val == PURC_VARIANT_INVALID) {
        goto failed;
    }
    if (!purc_variant_cast_to_number(val, &number, false) ||
    ...
    val = purc_variant_object_get_by_ckey(bdtime, _KN_hour, false);
    if (val == PURC_VARIANT_INVALID) {
        goto failed;
    }
    if (!purc_variant_cast_to_number(val, &number, false) ||
        ...
        bool silently)
{
    ...
    if (!get_broken_down_time(argv[0], &tm, &usec)) {
        goto failed;
    }

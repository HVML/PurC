        ...
        purc_variant_t jo)
{
    ...
    if ((jo_tmp = purc_variant_object_get_by_ckey_ex (jo, "retCode", true)) &&
    ...
    if (ret_code == PCRDR_SC_OK) {
        ...
        if (data) {
            ...
            tmp = purc_variant_object_get_by_ckey_ex(jo, "retCode", true);
            if (tmp)
                purc_variant_object_set_by_static_ckey(data, "retCode", tmp);

            tmp = purc_variant_object_get_by_ckey_ex(jo, "retMsg", true);
            if (tmp)
        ...
        purc_variant_t *jo)
{
    ...
    *jo = purc_variant_make_from_json_string(json, json_len);
    ...
    if ((jo_tmp = purc_variant_object_get_by_ckey_ex(*jo, "packetType", true))) {
        ...
        const char *payload, size_t len, char **challenge)
{
    ...
    if ((jo_tmp = purc_variant_object_get_by_ckey_ex(jo, "packetType", true))) {
        ...
        if (strcasecmp(pack_type, "error") == 0) {
            ...
            if ((jo_tmp = purc_variant_object_get_by_ckey_ex(jo,
                            "protocolName", true))) {
        ...
        const char* bubble_data)
{
    ...
    if ((jo_tmp = purc_variant_object_get_by_ckey_ex(jo, "bubbleName",
                    true)) &&
...
dispatch_result_packet(struct pcdvobjs_stream *stream, purc_variant_t jo)
{
    ...
    if ((jo_tmp = purc_variant_object_get_by_ckey_ex(jo, "resultId", true)) &&
            (result_id = purc_variant_get_string_const(jo_tmp))) {
    }
    else {
        PC_WARN("No resultId\n");
    ...
    if ((jo_tmp = purc_variant_object_get_by_ckey_ex(jo, "callId", true)) &&
            (call_id = purc_variant_get_string_const(jo_tmp))) {
    }
    else {
        set_error(ext, BADMSGPAYLOAD);
...
dispatch_event_packet(struct pcdvobjs_stream *stream, purc_variant_t jo)
{
    ...
    if ((jo_tmp = purc_variant_object_get_by_ckey_ex(jo, "fromEndpoint", true)) &&
            (from_endpoint = purc_variant_get_string_const(jo_tmp))) {
    }
    else {
        set_error(ext, BADMSGPAYLOAD);
    ...
    if ((jo_tmp = purc_variant_object_get_by_ckey_ex(jo, "fromBubble", true)) &&
    ...
    if ((jo_tmp = purc_variant_object_get_by_ckey_ex(jo, "eventId", true)) &&

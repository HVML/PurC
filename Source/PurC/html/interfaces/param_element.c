/**
 * @file param_element.c
 * @author 
 * @date 2021/07/02
 * @brief The complementation of html param element.
 *
...
pchtml_html_param_element_interface_create(pchtml_html_document_t *document)
{
    pchtml_html_param_element_t *element;

    element = pcutils_mraw_calloc(document->dom_document.mraw,
                                 sizeof(pchtml_html_param_element_t));
    if (element == NULL) {
        return NULL;
    }
...
pchtml_html_param_element_interface_destroy(pchtml_html_param_element_t *param_element)
{
    return pcutils_mraw_free(
        pcdom_interface_node(param_element)->owner_document->mraw,
        param_element);
}

 ...
 * @author 
 * @date 2021/07/02
 * @brief The complementation of html map element.
 *
...
{
    pchtml_html_map_element_t *element;

    element = pchtml_mraw_calloc(document->dom_document.mraw,
                                 sizeof(pchtml_html_map_element_t));
    if (element == NULL) {
        return NULL;
    }
...
pchtml_html_map_element_interface_destroy(pchtml_html_map_element_t *map_element)
{
    return pchtml_mraw_free(
        pchtml_dom_interface_node(map_element)->owner_document->mraw,
        map_element);
}

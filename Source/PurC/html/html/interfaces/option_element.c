/**
 * @file option_element.c
 * @author 
 * @date 2021/07/02
 * @brief The complementation of html option element.
 *
...
{
    pchtml_html_option_element_t *element;

    element = pchtml_mraw_calloc(document->dom_document.mraw,
                                 sizeof(pchtml_html_option_element_t));
    if (element == NULL) {
        return NULL;
    }
...
pchtml_html_option_element_interface_destroy(pchtml_html_option_element_t *option_element)
{
    return pchtml_mraw_free(
        pchtml_dom_interface_node(option_element)->owner_document->mraw,
        option_element);
}

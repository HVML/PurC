 ...
 * @author 
 * @date 2021/07/02
 * @brief The complementation of html menu element.
 *
...
{
    pchtml_html_menu_element_t *element;

    element = pchtml_mraw_calloc(document->dom_document.mraw,
                                 sizeof(pchtml_html_menu_element_t));
    if (element == NULL) {
        return NULL;
    }
...
pchtml_html_menu_element_interface_destroy(pchtml_html_menu_element_t *menu_element)
{
    return pchtml_mraw_free(
        pchtml_dom_interface_node(menu_element)->owner_document->mraw,
        menu_element);
}

 ...
 * @author 
 * @date 2021/07/02
 * @brief The complementation of html pre element.
 *
...
pchtml_html_pre_element_interface_create(pchtml_html_document_t *document)
{
    pchtml_html_pre_element_t *element;

    element = pchtml_mraw_calloc(document->dom_document.mraw,
                                 sizeof(pchtml_html_pre_element_t));
    if (element == NULL) {
        return NULL;
    }
...
pchtml_html_pre_element_t *
pchtml_html_pre_element_interface_destroy(pchtml_html_pre_element_t *pre_element)
{
    return pchtml_mraw_free(
        pchtml_dom_interface_node(pre_element)->owner_document->mraw,
        pre_element);
}

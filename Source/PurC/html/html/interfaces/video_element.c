/**
 * @file video_element.c
 * @author 
 * @date 2021/07/02
 * @brief The complementation of html video element.
 *
...
{
    pchtml_html_video_element_t *element;

    element = pchtml_mraw_calloc(document->dom_document.mraw,
                                 sizeof(pchtml_html_video_element_t));
    if (element == NULL) {
        return NULL;
    }
...
pchtml_html_video_element_interface_destroy(pchtml_html_video_element_t *video_element)
{
    return pchtml_mraw_free(
        pchtml_dom_interface_node(video_element)->owner_document->mraw,
        video_element);
}

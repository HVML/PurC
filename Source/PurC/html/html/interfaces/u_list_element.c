/**
 * @file u_list_element.c
 * @author 
 * @date 2021/07/02
 * @brief The complementation of html ul element.
 *
...
{
    pchtml_html_u_list_element_t *element;

    element = pchtml_mraw_calloc(document->dom_document.mraw,
                                 sizeof(pchtml_html_u_list_element_t));
    if (element == NULL) {
        return NULL;
    }
...
pchtml_html_u_list_element_interface_destroy(pchtml_html_u_list_element_t *u_list_element)
{
    return pchtml_mraw_free(
        pchtml_dom_interface_node(u_list_element)->owner_document->mraw,
        u_list_element);
}

/*
 * connect.c -- The implementation of API to manage PURCRDR connections.
 *
        ...
        const char* app_name, const char* runner_name, pcrdr_conn** conn)
{
    ...
    if ((fd = socket (AF_UNIX, SOCK_STREAM, 0)) < 0) {
        PC_DEBUG ("Failed to call `socket` in pcrdr_connect_via_unix_socket: %s\n",
                strerror (errno));
    ...
    if (bind (fd, (struct sockaddr *) &unix_addr, len) < 0) {
        PC_DEBUG ("Failed to call `bind` in pcrdr_connect_via_unix_socket: %s\n",
                strerror (errno));
        goto error;
    }
    if (chmod (unix_addr.sun_path, CLI_PERM) < 0) {
        PC_DEBUG ("Failed to call `chmod` in pcrdr_connect_via_unix_socket: %s\n",
                strerror (errno));
        goto error;
    }
    ...
    if (connect (fd, (struct sockaddr *) &unix_addr, len) < 0) {
        PC_DEBUG ("Failed to call `connect` in pcrdr_connect_via_unix_socket: %s\n",
                strerror (errno));
        goto error;
    }

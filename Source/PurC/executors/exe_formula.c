...
exe_formula_create(enum purc_exec_type type, purc_variant_t input, bool asc_desc)
{
...
static inline bool
exe_formula_parse_rule(purc_exec_inst_t inst, const char* rule)
{
...
exe_formula_choose(purc_exec_inst_t inst, const char* rule)
{
    ...
    if (!exe_formula_parse_rule(inst, rule))
        return PURC_VARIANT_INVALID;
...
exe_formula_destroy(purc_exec_inst_t inst)
{
    ...
    struct pcexec_exe_formula_inst *exe_formula_inst = (struct pcexec_exe_formula_inst*)inst;

    if (exe_formula_inst->super.input) {
        purc_variant_unref(exe_formula_inst->super.input);
        exe_formula_inst->super.input = PURC_VARIANT_INVALID;
    }
    if (exe_formula_inst->super.selected_keys) {
        purc_variant_unref(exe_formula_inst->super.selected_keys);
        exe_formula_inst->super.selected_keys = PURC_VARIANT_INVALID;
    }

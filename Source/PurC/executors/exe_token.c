...
exe_token_create(enum purc_exec_type type, purc_variant_t input, bool asc_desc)
{
    if (!purc_variant_is_object(input))
        return NULL;

    struct pcexec_exe_token_inst *inst;
...
static inline bool
exe_token_parse_rule(purc_exec_inst_t inst, const char* rule)
{
...
exe_token_choose(purc_exec_inst_t inst, const char* rule)
{
    ...
    if (!exe_token_parse_rule(inst, rule))
        return PURC_VARIANT_INVALID;
...
exe_token_destroy(purc_exec_inst_t inst)
{
    ...
    struct pcexec_exe_token_inst *exe_token_inst = (struct pcexec_exe_token_inst*)inst;

    if (exe_token_inst->super.input) {
        purc_variant_unref(exe_token_inst->super.input);
        exe_token_inst->super.input = PURC_VARIANT_INVALID;
    }
    if (exe_token_inst->super.selected_keys) {
        purc_variant_unref(exe_token_inst->super.selected_keys);
        exe_token_inst->super.selected_keys = PURC_VARIANT_INVALID;
    }

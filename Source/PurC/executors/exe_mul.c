...
static inline bool
exe_mul_parse_rule(purc_exec_inst_t inst, const char* rule)
{
...
exe_mul_choose(purc_exec_inst_t inst, const char* rule)
{
    ...
    if (!exe_mul_parse_rule(inst, rule))
        return PURC_VARIANT_INVALID;
...
exe_mul_destroy(purc_exec_inst_t inst)
{
    ...
    struct pcexec_exe_mul_inst *exe_mul_inst = (struct pcexec_exe_mul_inst*)inst;

    if (exe_mul_inst->super.input) {
        purc_variant_unref(exe_mul_inst->super.input);
        exe_mul_inst->super.input = PURC_VARIANT_INVALID;
    }
    if (exe_mul_inst->super.selected_keys) {
        purc_variant_unref(exe_mul_inst->super.selected_keys);
        exe_mul_inst->super.selected_keys = PURC_VARIANT_INVALID;
    }

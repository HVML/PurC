...
exe_char_create(enum purc_exec_type type, purc_variant_t input, bool asc_desc)
{
    if (!purc_variant_is_object(input))
        return NULL;

    struct pcexec_exe_char_inst *inst;
...
static inline bool
exe_char_parse_rule(purc_exec_inst_t inst, const char* rule)
{
...
exe_char_choose(purc_exec_inst_t inst, const char* rule)
{
    ...
    if (!exe_char_parse_rule(inst, rule))
        return PURC_VARIANT_INVALID;
...
exe_char_destroy(purc_exec_inst_t inst)
{
    ...
    struct pcexec_exe_char_inst *exe_char_inst = (struct pcexec_exe_char_inst*)inst;

    if (exe_char_inst->super.input) {
        purc_variant_unref(exe_char_inst->super.input);
        exe_char_inst->super.input = PURC_VARIANT_INVALID;
    }
    if (exe_char_inst->super.selected_keys) {
        purc_variant_unref(exe_char_inst->super.selected_keys);
        exe_char_inst->super.selected_keys = PURC_VARIANT_INVALID;
    }

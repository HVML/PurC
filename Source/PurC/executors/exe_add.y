 ...
 * @author
 * @date
 * @brief The implementation of public part for add.
 *
...
%code top {
    // here to include header files required for generated exe_add.tab.c
    #define _GNU_SOURCE
    #include <stddef.h>
    #include <stdio.h>
    #include <string.h>
...
%code requires {
    ...
    // especially, for struct exe_add_param
    // and parse function for example:
    // int exe_add_parse(const char *input,
    //        struct exe_add_param *param);
    // #include "exe_add.h"
    // here we define them locally
    struct exe_add_token {
        const char      *text;
        size_t           leng;
    };

    #define YYSTYPE       EXE_ADD_YYSTYPE
    #define YYLTYPE       EXE_ADD_YYLTYPE
...
%code {
    ...
    static void yyerror(
        ...
        struct exe_add_param *param,           // match %parse-param
        const char *errsg
    );
...
add_rule:
  ADD ':' number_comparing_logical_expression BY exp     { $$.ncle = $3; $$.nexp = $5; }
;

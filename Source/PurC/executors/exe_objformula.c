...
exe_objformula_create(enum purc_exec_type type,
...
exe_objformula_choose(purc_exec_inst_t inst, const char* rule)
{
    if (!inst || !rule) {
...
exe_objformula_it_begin(purc_exec_inst_t inst, const char* rule)
{
    if (!inst || !rule) {
        pcinst_set_error(PCEXECUTOR_ERROR_BAD_ARG);
        return NULL;
...
// 根据迭代子获得对应的变体值
static purc_variant_t
exe_objformula_it_value(purc_exec_inst_t inst, purc_exec_iter_t it)
{
    if (!inst || !it) {
        pcinst_set_error(PCEXECUTOR_ERROR_BAD_ARG);
        return NULL;
...
static purc_exec_iter_t
exe_objformula_it_next(purc_exec_inst_t inst, purc_exec_iter_t it, const char* rule)
{
    if (!inst || !it) {
        pcinst_set_error(PCEXECUTOR_ERROR_BAD_ARG);
        return NULL;
...
// 用于执行规约
static purc_variant_t
exe_objformula_reduce(purc_exec_inst_t inst, const char* rule)
{
    if (!inst || !rule) {
        pcinst_set_error(PCEXECUTOR_ERROR_BAD_ARG);
        return PURC_VARIANT_INVALID;
...
// 销毁一个执行器实例
static bool
exe_objformula_destroy(purc_exec_inst_t inst)
{
    if (!inst) {
        pcinst_set_error(PCEXECUTOR_ERROR_BAD_ARG);
        return false;
...
static struct purc_exec_ops exe_objformula_ops = {
    exe_objformula_create,
    exe_objformula_choose,
    exe_objformula_it_begin,
    exe_objformula_it_value,
    exe_objformula_it_next,
    exe_objformula_reduce,
    exe_objformula_destroy,
};

int pcexec_exe_objformula_register(void)
{
    bool ok = purc_register_executor("OBJFORMULA", &exe_objformula_ops);
    return ok ? 0 : -1;
}

 ...
 * @author
 * @date
 * @brief The implementation of public part for sub.
 *
...
%code top {
    // here to include header files required for generated exe_sub.tab.c
    #define _GNU_SOURCE
    #include <stddef.h>
    #include <stdio.h>
    #include <string.h>
...
%code requires {
    ...
    // especially, for struct exe_sub_param
    // and parse function for example:
    // int exe_sub_parse(const char *input,
    //        struct exe_sub_param *param);
    // #include "exe_sub.h"
    // here we define them locally
    struct exe_sub_token {
        const char      *text;
        size_t           leng;
    };

    #define YYSTYPE       EXE_SUB_YYSTYPE
    #define YYLTYPE       EXE_SUB_YYLTYPE
...
%code {
    ...
    static void yyerror(
        ...
        struct exe_sub_param *param,           // match %parse-param
        const char *errsg
    );
...
/* Bison declarations. */
%require "3.0.4"
%define api.prefix {exe_sub_yy}
%define api.pure full
%define api.token.prefix {TOK_EXE_SUB_}
%define locations
...
sub_rule:
  SUB ':' number_comparing_logical_expression BY exp     { $$.ncle = $3; $$.nexp = $5; }
;
...
{
    yyscan_t arg = {0};
    exe_sub_yylex_init(&arg);
    // exe_sub_yyset_in(in, arg);
    int debug_flex = param ? param->debug_flex : 0;
    int debug_bison = param ? param->debug_bison: 0;
    exe_sub_yyset_debug(debug_flex, arg);
    yydebug = debug_bison;
    // exe_sub_yyset_extra(param, arg);
    exe_sub_yy_scan_bytes(input ? input : "", input ? len : 0, arg);
    int ret =exe_sub_yyparse(arg, param);
    exe_sub_yylex_destroy(arg);
    return ret ? -1 : 0;
}

...
TEST(exe_char, basic)
{
    ...
    // initial purc
    int ret = purc_init ("cn.fmsoft.hvml.test", "exe_char", &info);
    ...
    ok = purc_register_executor("CHAR", &ops);
    EXPECT_FALSE(ok);
...
parse(const char *rule, char *err_msg, size_t sz_err_msg)
{
    ...
    bool r;
    r = exe_char_parse(rule, strlen(rule), &param) == 0;
    if (param.err_msg) {
...
TEST(exe_char, files)
{
    ...
    purc_instance_extra_info info = {};
    r = purc_init("cn.fmsoft.hvml.test", "exe_char", &info);
    EXPECT_EQ(r, PURC_ERROR_OK);
    if (r)
        return;

    const char *rel = "data/char.*.rule";

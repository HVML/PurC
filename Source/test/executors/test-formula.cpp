...
TEST(exe_formula, basic)
{
    purc_instance_extra_info info = {};
    bool cleanup = false;

    // initial purc
    int ret = purc_init("cn.fmsoft.hvml.test", "exe_formula", &info);
    ...
    ok = purc_register_executor("FORMULA", &ops);
    EXPECT_FALSE(ok);
...
parse(const char *rule, char *err_msg, size_t sz_err_msg)
{
    ...
    bool r;
    r = exe_formula_parse(rule, strlen(rule), &param) == 0;
    if (param.err_msg) {
...
TEST(exe_formula, files)
{
    ...
    purc_instance_extra_info info = {};
    r = purc_init("cn.fmsoft.hvml.test", "exe_formula", &info);
    EXPECT_EQ(r, PURC_ERROR_OK);
    if (r)
        return;

    const char *rel = "data/formula.*.rule";

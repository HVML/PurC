...
TEST(exe_mul, basic)
{
    ...
    // initial purc
    int ret = purc_init ("cn.fmsoft.hvml.test", "exe_mul", &info);
    ...
    ok = purc_register_executor("MUL", &ops);
    EXPECT_FALSE(ok);
...
parse(const char *rule, char *err_msg, size_t sz_err_msg)
{
    ...
    bool r;
    r = exe_mul_parse(rule, strlen(rule), &param) == 0;
    if (param.err_msg) {
...
TEST(exe_mul, files)
{
    ...
    purc_instance_extra_info info = {};
    r = purc_init("cn.fmsoft.hvml.test", "exe_mul", &info);
    EXPECT_EQ(r, PURC_ERROR_OK);
    if (r)
        return;

    const char *rel = "data/mul.*.rule";

...
TEST(exe_div, basic)
{
    ...
    // initial purc
    int ret = purc_init ("cn.fmsoft.hvml.test", "exe_div", &info);
    ...
    ok = purc_register_executor("DIV", &ops);
    EXPECT_FALSE(ok);
...
TEST(exe_div, files)
{
    ...
    purc_instance_extra_info info = {};
    r = purc_init("cn.fmsoft.hvml.test", "exe_div", &info);
    EXPECT_EQ(r, PURC_ERROR_OK);
    if (r)
        return;

    const char *rel = "data/div.*.rule";

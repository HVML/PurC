/*
 * @file test_quit_main_runner.c
 * @author Vincent Wei
 * @date 2022/07/21
 * @brief The program to test multiple runners; the following APIs are covered:
...
static int my_cond_handler(purc_cond_t event, void *arg, void *data)
{
    (void)data;

    if (event == PURC_COND_SHUTDOWN_ASKED) {
        struct pcrdr_msg *msg = (struct pcrdr_msg *)arg;
        purc_atom_t inst_mgr = purc_get_instmgr_rid();

        const char *src_ept = purc_variant_get_string_const(msg->sourceURI);
        int r = strcmp(src_ept, purc_atom_to_string(inst_mgr));
        assert(r == 0);
        fprintf(stdout, "Got shutdown request from InstMgr.");

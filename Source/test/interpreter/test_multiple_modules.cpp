/*
 * @file test-multiple-modules.cpp
 * @author Xue Shuming
 * @date 2025/03/22
 * @brief The program tests multiple modules 
 *
...
TEST(websocket, plain_server_plain_client)
{
    ...
    const char *rel = "modules/module-define-op-groups-on-runner.hvml";
    test_getpath_from_env_or_rel(path, sizeof(path),
        env, rel);

    char *url = purc_url_encode_alloc(path, true);
    char query[PATH_MAX];
    snprintf(query, PATH_MAX, "module=%s", url);
    free(url);

    run_one_comp_test("interpreter/modules/page-load-op-groups.hvml", query);

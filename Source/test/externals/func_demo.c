...
get_member(purc_variant_t on_value, purc_variant_t with_value)
{
    ...
    purc_variant_t v = purc_variant_object_get_ex(on_value, with_value, true);
...
statsUserRegion(purc_variant_t on_value, purc_variant_t with_value)
{
    ...
    for (size_t i = 0; i < nr_size; i++) {
        ...
        purc_variant_t mv = purc_variant_object_get_ex(member, with_value, true);
        if (mv) {
            purc_variant_t counter = purc_variant_object_get_ex(regions, mv, true);
            uint64_t count = 0;
            if (counter) {
            ...
            purc_variant_t counter = purc_variant_object_get_ex(regions,
                    unknown_key, true);
            uint64_t count = 0;
            if (counter) {
                purc_variant_cast_to_ulongint(counter, &count, false);

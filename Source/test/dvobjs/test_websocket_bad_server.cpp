/*
 * @file test-websocket-bad-server.c
 * @author Vincent Wei
 * @date 2025/04/12
...
TEST(websocket, plain_server_plain_client)
{
    ...
    run_one_comp_test("dvobjs/socket/inet-websocket-bad-server.hvml", "secure=false&client=plain");
...
TEST(websocket, secure_server_secure_client)
{
    ...
    run_one_comp_test("dvobjs/socket/inet-websocket-bad-server.hvml", query);
    free(query);

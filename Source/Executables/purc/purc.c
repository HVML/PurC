...
#define KEY_FLAG_PRINT_DOCS     "printDocs"
#define KEY_FLAG_PRINT_RSLT     "printRslt"
...
static int read_option_args(struct my_opts *opts, int argc, char **argv)
{
    ...
    static const struct option long_opts[] = {
        ...
        { "print-docs"                  , no_argument       , NULL , 'p' },
        { "print-result"                , no_argument       , NULL , 't' },
    ...
    while ((o = getopt_long(argc, argv, short_options, long_opts, &idx)) >= 0) {
        ...
        case 'p':
            opts->print_docs = true;
            break;

        case 't':
            opts->print_rslt = true;
            break;
...
transfer_opts_to_variant(struct my_opts *opts, purc_variant_t request)
{
    ...
    tmp = purc_variant_make_boolean(opts->print_docs);
    purc_variant_object_set_by_static_ckey(run_info.opts,
            KEY_FLAG_PRINT_DOCS, tmp);
    purc_variant_unref(tmp);

    tmp = purc_variant_make_boolean(opts->print_rslt);
    purc_variant_object_set_by_static_ckey(run_info.opts,
            KEY_FLAG_PRINT_RSLT, tmp);
    purc_variant_unref(tmp);
        ...
        purc_variant_t app, purc_variant_t runner, purc_variant_t coroutines)
{
    ...
    for (size_t i = 0; i < nr_coroutines; i++) {
        ...
        if (vdom == NULL) {
            if (opts->verbose) {
                fprintf(stderr, "Failed to parse HVML from %s for crtn[%u]\n",
                        url, (unsigned)i);
                ...
                fprintf(stderr, "Failed to parse HVML from %s for crtn[%u]: %s\n",
                        url, (unsigned)i,
                        purc_get_error_message(purc_get_last_error()));
            }
            continue;
        ...
        void *data)
{
    if (event == PURC_COND_COR_EXITED) {
        ...
        /* Always print the error messages since 0.9.24 */
        struct crtn_info *crtn_info = purc_coroutine_get_user_data(cor);
        if (crtn_info) {
            fprintf(stdout,
